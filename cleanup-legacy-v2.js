...
function fixAboutPage() {
  patch('frontend/src/app/about/page.tsx', content => {
    let out = content;
    out = out.replace(/declare module \"next\/navigation\"\s*\{[\s\S]*?\}\s*/g, '');
    out = out.replace(/import \{ useRouter\s*\} from \"next\/navigation\";/g, 'import { useRouter } from "next/navigation";');
    const replacements = [
      ...
      ['sm:w-[380px]', 'sm:w-95'],
      ['from-violet-400 via-fuchsia-400 to-cyan-300', 'from-violet-400 via-fuchsia-400 to-cyan-300'],
    ];
...
function fixAuthPackage() {
  patch('services/auth/package.json', content => {
    let out = content;
    out = out.replace(/"multer": "\^2\.2\.0",\s*\n\s*\}/, '"multer": "^2.2.0"\n  }');
    out = out.replace(/"resend": "\^6\.19\.0",?\s*\n/, '');
    return out;
  });
}

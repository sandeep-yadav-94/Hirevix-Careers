...
function MetricCard({ label, value, detail, icon: Icon, tone = "blue" }: { label: string; value: string | number; detail: string; icon: typeof BriefcaseBusiness; tone?: "blue" | "violet" | "emerald" | "amber" }) {
  ...
  return <div className="rounded-[24px] border border-slate-200/80 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{value}</p><p className="mt-2 text-xs font-medium text-slate-500">{detail}</p></div><div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${tones[tone]}`}><Icon size={20} /></div></div></div>;
}
...
function JobSeekerDashboard({ jobs, applications }: { jobs: DashboardJob[]; applications: Application[] }) {
  ...
  return <section className="space-y-5"><div className="flex flex-col gap-4 rounded-[28px] border border-slate-200 bg-slate-950 p-6 text-white shadow-[0_24px_60px_-32px_rgba(15,23,42,0.8)] sm:flex-row sm:items-end sm:justify-between"><div><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm text-slate-200"><Sparkles size={15} className="text-blue-300" /> Job seeker dashboard</div><h2 className="mt-3 text-2xl font-semibold">Your career search, beautifully organized.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Track applications, discover fitting opportunities, and keep your profile ready for the next recruiter.</p></div><Link href="/jobs"><Button className="gap-2 bg-white text-slate-950 hover:bg-slate-100">Explore jobs <ChevronRight size={16} /></Button></Link></div>
...
function RecruiterDashboard({ jobs, applications }: { jobs: DashboardJob[]; applications: Application[] }) {
  ...
  return <section className="space-y-5"><div className="flex flex-col gap-4 rounded-[28px] border border-slate-200 bg-[linear-gradient(135deg,#0f172a_0%,#172554_100%)] p-6 text-white shadow-[0_24px_60px_-32px_rgba(15,23,42,0.8)] sm:flex-row sm:items-end sm:justify-between"><div><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm text-slate-200"><Sparkles size={15} className="text-blue-300" /> Recruiter dashboard</div><h2 className="mt-3 text-2xl font-semibold">Your hiring pipeline, in one focused view.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Stay close to every open role, candidate, and decision without losing momentum.</p></div><Link href="#companies"><Button className="gap-2 bg-white text-slate-950 hover:bg-slate-100">Manage roles <ChevronRight size={16} /></Button></Link></div>

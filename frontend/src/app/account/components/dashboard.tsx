...
function JobSeekerDashboard({ jobs, applications }: { jobs: DashboardJob[]; applications: Application[] }) {
  ...
  const notifications = applications.filter((application) => application.recruiter_note || application.status !== "Submitted");
...
function NotificationPanel({ notifications, recruiter = false }: { notifications: Application[]; recruiter?: boolean }) {
  useEffect(() => {
    ...
    if (!targetId) return;
    const timer = window.setTimeout(() => document.getElementById(targetId)?.scrollIntoView({ behavior: "smooth", block: "center" }), 150);
    return () => window.clearTimeout(timer);
  }, [notifications]);

  return <div className="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-2"><Bell size={18} className="text-amber-500" /><div><p className="font-semibold text-slate-950">Notifications</p><p className="text-sm text-slate-500">{recruiter ? "Candidates that need attention." : "Updates from your applications."}</p></div></div><div className="mt-5 space-y-3">{notifications.length ? notifications.map((application) => <div id={recruiter ? undefined : `application-${application.application_id}`} key={application.application_id} className="flex items-start justify-between gap-3 rounded-2xl border border-slate-200 p-4 transition-colors target:border-blue-400 target:bg-blue-50 target:ring-4 target:ring-blue-100"><div><p className="font-semibold text-slate-900">{recruiter ? `${application.applicant_name || application.applicant_email} applied` : application.job_title}</p><p className="mt-1 text-sm text-slate-500">{recruiter ? "Review the candidate profile and resume." : application.recruiter_note || `Your application status is ${application.status}.`}</p></div><span className={`shrink-0 rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(application.status)}`}>{application.status}</span></div>) : <Empty text="You are all caught up. New updates will appear here." />}</div></div>;
}

             ...
             <div className="absolute -bottom-16 left-8">
                <div className="relative group">
                   <div className="w-32 h-32 rounded-full border-4 border-background overflow-hidden shadow-xl bg-background">
                      <img src={user.profile_pic ? user.profile_pic : "/user-avatar.webp"} alt="" className='w-full h-full object-cover'/>
                   </div>
                   
                   {/*  edit options for your*/}

                   {
                     isYourAccount && (<><Button variant={"secondary"} size={"icon"} onClick={handleClick} className='absolute bottom-0 right-0 rounded-full h-10 w-10 shadow-lg '>
                        <Camera size={18} />
                     </Button>
            ...
            <div className="flex items-start justify-between flex-wrap gap-4">
               <div className="space-y-1 ">
                  <div className="flex items-center gap-3">
                     ...
                     {/* edit button */}
                     { isYourAccount && (
                        <Button variant={"ghost"} size={"icon"} className='h-8 w-8' onClick={handleEditClick}><Edit size={16} /></Button>
                     )}
                  </div>
               ...
               { user.role === "jobseeker" && user.resume && <div className='mt-8'>
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-lg border hover:border-blue-500 transition-colors">
                     ...
                     <div className="flex-1 ">
                        ...
                        <Link href={user.resume} className='text-sm text-blue-500 hover:underline' target="_blank">Check Resume PDF</Link>
                     </div>
                     {/* editbutton */}
